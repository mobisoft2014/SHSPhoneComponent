...
+(BOOL) logicTextField:(SHSPhoneTextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    NSInteger caretPosition = [self pushCaretPosition:textField range:range allowAnyChar:textField.allowAnyCharacter];
    BOOL isDeleting = (string.length == 0);
    NSString *newString;
    if (isDeleting) {
        newString = [SHSPhoneNumberFormatter formattedRemove:textField.text AtIndex:range allowAnyChar:textField.allowAnyCharacter];
    } else {
    ...
    [self popCaretPosition:textField range:range caretPosition:caretPosition allowAnyChar:textField.allowAnyCharacter];
...
+(NSInteger) pushCaretPosition:(UITextField *)textField range:(NSRange)range allowAnyChar:(BOOL)value
{
    NSString *subString = [textField.text substringFromIndex:range.location + range.length];
    return [SHSPhoneNumberFormatter valuableCharCountIn:subString allowAnyChar:value];
}

+(void) popCaretPosition:(UITextField *)textField range:(NSRange)range caretPosition:(NSInteger)caretPosition allowAnyChar:(BOOL)value
{
    ...
    for (NSInteger index = [text length] - 1; index >= 0 && lasts > 0; index--) {
        ...
        if ([SHSPhoneNumberFormatter isValuableChar:ch allowAnyChar:value]) {
            lasts--;
        }
